...
function Search_Virus(){
	...
	for (( i=0; i < count_cm_algorithm; i++ )); do
		...
		local result=0
		local current_list_files=$list_files_with_php_code
		local current_count_files=$count_files_with_php_code
...
#		Print_String "\n$(Design "BOLD" "[$(( $i+1 ))/$count_cm_algorithm] ") $(Design "MB" "$child_id") (из результатов $parent_id)"
#	Print_String "\n$(Design "BOLD" "[$(( $i+1 ))/$count_cm_algorithm] ") $(Design "MB" "${array_split_CM_ID[i]}")"
		...
		while read file_path; do
			...
			# sv2=из
			echo -ne "\r$sv1 $progress_bar_count $sv2 $current_count_files [$(Design "MB" "$result")]"
